...
static NSString * const coreDataModelName = @"Goods";
//获取CodeData
static NSString * const coreDataEntityName = @"GoodsCodeDataMode";
//数据库
static NSString * const sqliteName = @"Goods.sqlite";
...
// 查询数据
- (void)selectEntitySuccess:(void(^)(NSArray *results))success fail:(void(^)(NSError *error))fail {
//    // 1.初始化一个查询请求
//    NSFetchRequest *request = [[NSFetchRequest alloc] init];
//    // 2.设置要查询的实体
//    NSEntityDescription *desc = [NSEntityDescription entityForName:coreDataEntityName inManagedObjectContext:self.managedObjectContext];
    ...
    //创建查询请求
    NSFetchRequest *request = [NSFetchRequest fetchRequestWithEntityName:coreDataEntityName];
    //发送查询请求
    NSArray *resArray = [self.managedObjectContext executeFetchRequest:request error:nil];
    if (resArray.count && success) {
        success(resArray);
    }
}

- (void)insertNewModel:(GoodModel *)entity success:(void (^)(void))success fail:(void (^)(NSError *))fail {
    // 通过传入上下文和实体名称，创建一个名称对应的实体对象（相当于数据库一组数据，其中含有多个字段）
    GoodModel * model = (GoodModel *)[NSEntityDescription insertNewObjectForEntityForName:coreDataEntityName inManagedObjectContext:self.managedObjectContext];
    
    model.idd = entity.idd;
    model.bigIcon = entity.bigIcon;
    ...
    model.isCollect = entity.isCollect;
    model.orderType = entity.orderType;
    
    model.commTitle = entity.commTitle;
    model.commDate = entity.commDate;
    model.commIcon = entity.commIcon;
    model.commName = entity.commName;
    
    NSLog(@"idd=%ld   commName=%@",model.idd, model.commName);
...
- (void)updateDataWithGoodsModel:(GoodModel *)entity {
    //创建查询请求
    NSFetchRequest *request = [NSFetchRequest fetchRequestWithEntityName:coreDataEntityName];
    //发送查询请求
    NSArray *resArray = [self.managedObjectContext executeFetchRequest:request error:nil];
    
    for (GoodModel *model in resArray) {
        if (model.idd == entity.idd) {
            model.idd = entity.idd;
            model.bigIcon = entity.bigIcon;
            model.date = entity.date;
            model.icon = entity.icon;
...
- (void)deleteAllDataSuccess:(void (^)(void))success fail:(void (^)(NSError *))fail {
    //创建删除请求
    NSFetchRequest *deleRequest = [NSFetchRequest fetchRequestWithEntityName:coreDataEntityName];
    //返回需要删除的对象数组
    NSArray *deleArray = [self.managedObjectContext executeFetchRequest:deleRequest error:nil];
    
    //从数据库中删除
    for (GoodModel *model in deleArray) {
        [self.managedObjectContext deleteObject:model];
    }
    NSError *error = nil;
    //保存--记住保存
